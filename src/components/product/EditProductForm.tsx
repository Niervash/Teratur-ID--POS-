...
import { Plus, X, Check, Trash2, Package, Save, ChevronDown } from 'lucide-react';
...
export const EditProductForm = ({ product, rawMaterials, onSaveProduct, onClose }: EditProductFormProps) => {
  ...
  useEffect(() => {
    ...
    const initialIngredients = product.ingredients.map(ing => {
      const material = rawMaterials.find(m => m.name === ing.name);
      return {
                      ...
                      >
                        <div className="flex-1 grid grid-cols-4 gap-3">
                          <Select
                            ...
                            dropdownStyle={{ borderRadius: '12px', overflow: 'hidden' }}
                            suffixIcon={<ChevronDown className="w-3 h-3" />}
                            options={availableMaterials.map(m => ({ value: m.id, label: m.name }))}
                          />
                          <Input
                            type="number"
                            value={ing.amount || ''}
            ...
            >
                ...
                <div className="space-y-2">
                  ...
                  <div className="border-t border-border pt-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-lg number-display text-primary">{formatCurrency(totalHpp)}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="glass-card p-4 text-center">
                  ...
                  <p className={`text-2xl font-bold number-display ${margin > 0 ? 'text-success' : 'text-destructive'}`}>
                    {margin.toFixed(1)}%
                  </p>
                </div>
                <div className="glass-card p-4 text-center">
                  <p className="text-sm text-muted-foreground mb-1">Profit per Unit</p>
                  <p className={`text-2xl font-bold number-display ${formData.sellingPrice - totalHpp > 0 ? 'text-success' : 'text-destructive'}`}>
                    {formatCurrency(formData.sellingPrice - totalHpp)}
                  </p>
                </div>
              </div>
